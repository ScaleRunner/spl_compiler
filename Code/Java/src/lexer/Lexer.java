...
public class Lexer {
	...
	public Token nextToken() {
		...
		if (match('=')) {
			...
			if (match('=')) {
				currentPosition++;
				return new Token(TokenType.TOK_EQ);
			}
		...
		if (match('>')) {
			...
			// Otherwise, we've seen nothing.
			return new Token(TokenType.TOK_GT);
		}
		...
		if (match('<')) {
			...
			// Otherwise, we've seen nothing.
			return new Token(TokenType.TOK_LT);
		}
		...
		if (match('&')) {
			...
			// Otherwise, we've seen nothing.
			return new Token(TokenType.TOK_ERR);
		}
		...
		if (match(':')) {
			...
			if (match(':')) {
				currentPosition++;
				return new Token(TokenType.TOK_FUNC_TYPE_DEF);
			}
			
			// Otherwise, we've seen nothing.
			return new Token(TokenType.TOK_CONS);
		}
		...
		if (match('{')) {
			...
			return new Token(TokenType.TOK_OPEN_CURLY);
		}
		
		if (match('}')) {
			currentPosition++;
			
			return new Token(TokenType.TOK_CLOSE_CURLY);
		}
		
		if (match('[')) {
			currentPosition++;
			
			return new Token(TokenType.TOK_OPEN_BRACKETS);
		}
		
		if (match(']')) {
			currentPosition++;
			
			return new Token(TokenType.TOK_CLOSE_BRACKETS);
		}
		
		if (match('(')) {
			currentPosition++;
			
			return new Token(TokenType.TOK_OPEN_PARENTESIS);
		}
		
		if (match(')')) {
			currentPosition++;
			
			return new Token(TokenType.TOK_CLOSE_PARENTESIS);
		}
		...
		if (match(';')) {
			...
			return new Token(TokenType.TOK_SEMI_COLON);
		}
	...
	Token lexIdentifier() {
		...
		if (result.equals("Int")) {
			return new Token(TokenType.TOK_KW_INT);
		}
		
		if (result.equals("Bool")) {
			return new Token(TokenType.TOK_KW_BOOL);
		}
		
		if (result.equals("Char")) {
			return new Token(TokenType.TOK_KW_CHAR);
		}
		...
		if (result.equals("->")) {
			return new Token(TokenType.TOK_KW_ARROW);
		}
